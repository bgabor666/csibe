...
def copy_result_files(result_origin_dir, csibe_results_path, llvm_revision, clang_revision):
    ...
    for subdir in os.listdir(result_origin_dir):
        ...
        shutil.copyfile(os.path.join(result_origin_dir, subdir, "all_results.csv"),
                        os.path.join(target_dir, "{}-{}-llvm{}-clang{}-results.csv".format(date_with_dashes, subdir, llvm_revision, clang_revision)))

        print "{}-{}-llvm{}-clang{}".format(date_with_dashes, subdir, llvm_revision, clang_revision)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clang-svn-path")

    args = parser.parse_args()

    llvm_revision = subprocess.check_output(["svnversion", args.llvm_svn_path])
    clang_revision = subprocess.check_output(["svnversion", args.clang_svn_path])

    copy_result_files(args.result_origin_dir, args.csibe_results_path, llvm_revision.rstrip(), clang_revision.rstrip())
